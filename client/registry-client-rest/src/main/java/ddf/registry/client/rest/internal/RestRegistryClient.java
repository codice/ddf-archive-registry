 ...
 *
 **/
package ddf.registry.client.rest.internal;
 ...
 */
public class RestRegistryClient {

    private static final Logger LOGGER = LoggerFactory.getLogger(RestRegistryClient.class);
    ...
    public RestRegistryClient(ConfigurationAdmin configAdmin,
            DynamicServiceResolver serviceResolver) {
    ...
    public void setRegistryUrl(String url) {
        if (StringUtils.isNotBlank(url)) {
            ...
            try {
                ...
                LOGGER.warn("Encountered an error while trying to connect to the remote registry.",
                        e);
            } finally {
    ...
    private void clearConfigurations() {
        for (Configuration config : configurationList) {
            try {
                ...
                LOGGER.debug(
                        "Configuration was already deleted, ignore error and continuing deletions.",
                        ise);
            }
        }
